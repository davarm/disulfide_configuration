...
for filename in os.listdir(path):
    ...
    amino_array=np.zeros(shape=(20,20))
    for line in get:
            amino_array[amino_dict[line[0]],amino_dict[line[2]]]=amino_array[amino_dict[line[0]],amino_dict[line[2]]]+1
            
    
    if np.sum(amino_array) > 14:
        amino_array=amino_array/np.sum(amino_array)
        plt.imshow(amino_array, cmap=cm.RdBu, interpolation='nearest')
        plt.colorbar()
        plt.clim(0.01, 0.3)
        plt.savefig(config+'.png')
        plt.patch.set_hatch('x')
        plt.show()
        plt.close()
    """            first.append(line[0])
